...
export function Article({ article }: ArticleProps) {
  ...
  return <article className="min-h-screen bg-brand-bg px-6 pb-24 pt-32 text-white"><div className="mx-auto max-w-3xl"><Link to="/blog" className="inline-flex items-center gap-2 text-sm text-gray-400 transition hover:text-brand-accent"><ArrowLeft className="h-4 w-4" />返回笔记列表</Link><header className="mt-10 border-b border-brand-border pb-10"><div className="flex flex-wrap gap-2">{article.tags.map((tag) => <span key={tag} className="bg-brand-accent/10 px-2.5 py-1 text-xs text-brand-accent">{tag}</span>)}</div><h1 className="mt-6 text-3xl font-bold leading-tight tracking-tight md:text-4xl">{article.title}</h1><p className="mt-5 flex items-center gap-2 text-sm text-gray-500"><Clock className="h-4 w-4" />{article.readTime}</p></header><div className="mt-12">{blocks.map((block, blockIndex) => block.startsWith("```") ? <pre key={blockIndex} className="mt-6 overflow-x-auto border border-brand-border bg-brand-code p-4 text-sm leading-6 text-gray-300"><code>{block.replace(/^```[^\n]*\n?/, "").replace(/```$/, "")}</code></pre> : block.split("\n").map((line, lineIndex) => renderLine(line, blockIndex * 1000 + lineIndex)))}</div></div></article>;
}
